    ...
    export namespace bo {
        ...
        export class SalesReturn extends ibas.BODocument<SalesReturn> implements ISalesReturn, ibas.IConvertedData {
            ...
            get documentTaxTotal(): number {
                return this.getProperty<number>(SalesReturn.PROPERTY_DOCUMENTTAXTOTAL_NAME);
            }
            /** 设置-单据税总计 */
            set documentTaxTotal(value: number) {
                this.setProperty(SalesReturn.PROPERTY_DOCUMENTTAXTOTAL_NAME, value);
            }
            ...
            protected registerRules(): ibas.IBusinessRule[] {
                return [
                    ...
                    new ibas.BusinessRuleSummation(
                        SalesReturn.PROPERTY_DOCUMENTTAXTOTAL_NAME, SalesReturn.PROPERTY_ITEMSTAXTOTAL_NAME, SalesReturn.PROPERTY_SHIPPINGSTAXTOTAL_NAME),
                    // 单据总计 = 行总计 + 运输费用
                    new ibas.BusinessRuleSummation(
                        SalesReturn.PROPERTY_DOCUMENTTOTAL_NAME, SalesReturn.PROPERTY_DISCOUNTTOTAL_NAME, SalesReturn.PROPERTY_SHIPPINGSEXPENSETOTAL_NAME),
                    // 小数舍入（单据总计）
            ...
            afterParsing(): void {
                ...
                for (let rule of ibas.businessRulesManager.getRules(ibas.objects.typeOf(this))) {
                    if (rule instanceof ibas.BusinessRuleSumElements) {
                        ...
                        && rule.result === SalesReturn.PROPERTY_DOCUMENTTAXTOTAL_NAME) {
                        rule.execute(this);
                    }
                }
            }
            ...
            protected registerRules(): ibas.IBusinessRule[] {
                return [
                    ...
                    new BusinessRuleDeductionDiscountPrice(
                        SalesReturnItem.PROPERTY_DISCOUNT_NAME, SalesReturnItem.PROPERTY_UNITPRICE_NAME, SalesReturnItem.PROPERTY_PRETAXPRICE_NAME
                        , ibas.config.get(ibas.CONFIG_ITEM_DECIMAL_PLACES_PRICE)
                    ),
                    // 计算税后价格 = 税前价格 * （1 + 税率）
                    new BusinessRuleDeductionTaxPrice(
                        SalesReturnItem.PROPERTY_TAXRATE_NAME, SalesReturnItem.PROPERTY_PRETAXPRICE_NAME, SalesReturnItem.PROPERTY_PRICE_NAME
                        , ibas.config.get(ibas.CONFIG_ITEM_DECIMAL_PLACES_PRICE)),
                    // 计算总计 = 数量 * 价格
                    new ibas.BusinessRuleMultiplication(
                    ...
                    // 计算税前总计 = 数量 * 税前价格
                    new ibas.BusinessRuleMultiplication(
                        SalesReturnItem.PROPERTY_PRETAXLINETOTAL_NAME, SalesReturnItem.PROPERTY_QUANTITY_NAME, SalesReturnItem.PROPERTY_PRETAXPRICE_NAME
                        , ibas.config.get(ibas.CONFIG_ITEM_DECIMAL_PLACES_SUM)),
                    // 计算税总额 = 稅前总计 * 税率
                    new ibas.BusinessRuleMultiplication(
                        SalesReturnItem.PROPERTY_TAXTOTAL_NAME, SalesReturnItem.PROPERTY_PRETAXLINETOTAL_NAME, SalesReturnItem.PROPERTY_TAXRATE_NAME
                        , ibas.config.get(ibas.CONFIG_ITEM_DECIMAL_PLACES_SUM)),
                ];
            }
            /** 重置 */
