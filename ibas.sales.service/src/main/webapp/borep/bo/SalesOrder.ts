        ...
        export class SalesOrderItem extends ibas.BODocumentLine<SalesOrderItem> implements ISalesOrderItem {
            ...
            protected registerRules(): ibas.IBusinessRule[] {
                return [
                    ...
                    // 计算总计（含税） = 数量 * 价格（含税）
                    new BusinessRuleDeductionPriceQtyTotal(
                        SalesOrderItem.PROPERTY_LINETOTAL_NAME, SalesOrderItem.PROPERTY_PRICE_NAME, SalesOrderItem.PROPERTY_QUANTITY_NAME
                        , ibas.config.get(ibas.CONFIG_ITEM_DECIMAL_PLACES_SUM)),
                    // 计算折扣后总计 = 折扣前总计 * 折扣
                    ...
                    new BusinessRuleDeductionLineTotal(
                        SalesOrderItem.PROPERTY_LINETOTAL_NAME, SalesOrderItem.PROPERTY_PRETAXLINETOTAL_NAME, SalesOrderItem.PROPERTY_TAXTOTAL_NAME
                        , ibas.config.get(ibas.CONFIG_ITEM_DECIMAL_PLACES_SUM)),
                ];
            }
            /** 重置 */
