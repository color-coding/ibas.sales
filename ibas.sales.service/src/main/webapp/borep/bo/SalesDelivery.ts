...
export class SalesDelivery extends BODocument<SalesDelivery> implements ISalesDelivery {
    ...
    baseDocument(document: ISalesOrder): void {
        if (objects.instanceOf(document, SalesOrder)) {
            if (!strings.equals(this.customerCode, document.customerCode)) {
                return;
            }
            // 复制行项目
...
export class SalesDeliveryItem extends BODocumentLine<SalesDeliveryItem> implements ISalesDeliveryItem {
    ...
    get materialBatches(): MaterialBatchItems {
        return this.getProperty<MaterialBatchItems>(SalesDeliveryItem.PROPERTY_MATERIALBATCHES_NAME);
    }
    /** 设置-物料批次集合 */
    ...
    get materialSerials(): MaterialSerialItems {
        return this.getProperty<MaterialSerialItems>(SalesDeliveryItem.PROPERTY_MATERIALSERIALS_NAME);
    }
    /** 设置-物料序列集合 */
