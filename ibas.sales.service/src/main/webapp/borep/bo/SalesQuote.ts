        ...
        export class SalesQuote extends ibas.BODocument<SalesQuote> implements ISalesQuote, ibas.IConvertedData {
            ...
            protected registerRules(): ibas.IBusinessRule[] {
                return [
                    ...
                    new BusinessRuleDeductionDocumentTotal(SalesQuote.PROPERTY_DOCUMENTTOTAL_NAME, SalesQuote.PROPERTY_DISCOUNTTOTAL_NAME),
                    // 小数舍入（单据总计）
        ...
        export class SalesQuoteItem extends ibas.BODocumentLine<SalesQuoteItem> implements ISalesQuoteItem {
            ...
            protected registerRules(): ibas.IBusinessRule[] {
                return [
                    ...
                    // 计算总计（含税） = 数量 * 价格（含税）
                    new BusinessRuleDeductionPriceQtyTotal(
                        SalesQuoteItem.PROPERTY_LINETOTAL_NAME, SalesQuoteItem.PROPERTY_PRICE_NAME, SalesQuoteItem.PROPERTY_QUANTITY_NAME
                        , ibas.config.get(ibas.CONFIG_ITEM_DECIMAL_PLACES_SUM)),
                    // 计算折扣后总计 = 折扣前总计 * 折扣
                    ...
                    new BusinessRuleDeductionLineTotal(
                        SalesQuoteItem.PROPERTY_LINETOTAL_NAME, SalesQuoteItem.PROPERTY_PRETAXLINETOTAL_NAME, SalesQuoteItem.PROPERTY_TAXTOTAL_NAME
                        , ibas.config.get(ibas.CONFIG_ITEM_DECIMAL_PLACES_SUM)),
                ];
            }
            /** 重置 */
