    ...
    export namespace bo {
        ...
        export class SalesQuote extends ibas.BODocument<SalesQuote> implements ISalesQuote, ibas.IConvertedData {
            ...
            get documentTaxTotal(): number {
                return this.getProperty<number>(SalesQuote.PROPERTY_DOCUMENTTAXTOTAL_NAME);
            }
            /** 设置-单据税总计 */
            set documentTaxTotal(value: number) {
                this.setProperty(SalesQuote.PROPERTY_DOCUMENTTAXTOTAL_NAME, value);
            }
            ...
            afterParsing(): void {
                ...
                for (let rule of ibas.businessRulesManager.getRules(ibas.objects.typeOf(this))) {
                    if (rule instanceof ibas.BusinessRuleSumElements) {
                        ...
                        && rule.result === SalesQuote.PROPERTY_DOCUMENTTAXTOTAL_NAME) {
                        rule.execute(this);
                    }
                }
            }
            ...
            protected registerRules(): ibas.IBusinessRule[] {
                return [
                    ...
                    new BusinessRuleDeductionDiscountPrice(
                        SalesQuoteItem.PROPERTY_DISCOUNT_NAME, SalesQuoteItem.PROPERTY_UNITPRICE_NAME, SalesQuoteItem.PROPERTY_PRETAXPRICE_NAME
                        , ibas.config.get(ibas.CONFIG_ITEM_DECIMAL_PLACES_PRICE)
                    ),
                    // 计算税后价格 = 税前价格 * （1 + 税率）
                    new BusinessRuleDeductionTaxPrice(
                        SalesQuoteItem.PROPERTY_TAXRATE_NAME, SalesQuoteItem.PROPERTY_PRETAXPRICE_NAME, SalesQuoteItem.PROPERTY_PRICE_NAME
                        , ibas.config.get(ibas.CONFIG_ITEM_DECIMAL_PLACES_PRICE)),
                    // 计算总计 = 数量 * 价格
                    new ibas.BusinessRuleMultiplication(
                    ...
                    // 计算税总额 = 稅前总计 * 税率
                    new ibas.BusinessRuleMultiplication(
                        SalesQuoteItem.PROPERTY_TAXTOTAL_NAME, SalesQuoteItem.PROPERTY_PRETAXLINETOTAL_NAME, SalesQuoteItem.PROPERTY_TAXRATE_NAME
                        , ibas.config.get(ibas.CONFIG_ITEM_DECIMAL_PLACES_SUM)),
                ];
            }
            /** 重置 */
