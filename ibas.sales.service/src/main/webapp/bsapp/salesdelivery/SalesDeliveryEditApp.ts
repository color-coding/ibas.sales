    ...
    export namespace app {
        ...
        export class SalesDeliveryEditApp extends ibas.BOEditApplication<ISalesDeliveryEditView, bo.SalesDelivery> {
            ...
            private chooseSalesDeliveryCustomer(): void {
                if (!ibas.objects.isNull(this.editData) && this.editData.salesDeliveryItems.where(c => !ibas.strings.isEmpty(c.baseDocumentType)).length > 0) {
                                                ...
                                                onCompleted(action: ibas.emMessageAction): void {
                                                    if (action !== ibas.emMessageAction.YES) {
                                                        ...
                                                        if (ibas.objects.isNull(parentItem)) {
                                                            parentItem = that.editData.salesDeliveryItems.create();
                                                        }
                                                        // 父项
