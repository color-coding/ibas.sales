    ...
    export namespace app {
        ...
        export class SalesCreditNoteEditApp extends ibas.BOEditService<ISalesCreditNoteEditView, bo.SalesCreditNote> {
            ...
            protected registerView(): void {
                ...
                this.view.chooseSalesCreditNoteItemMaterialCatalogEvent = this.chooseSalesCreditNoteItemMaterialCatalog;
            ...
            protected chooseSalesCreditNoteItemMaterialCatalog(caller: bo.SalesCreditNoteItem, filterConditions?: ibas.ICondition[]): void {
                ...
                ibas.servicesManager.runChooseService<materials.bo.BusinessPartnerMaterialCatalog>({
                    ...
                    onCompleted: (selecteds) => {
                        let count: number = this.editData.salesCreditNoteItems.length;
                        for (let selected of selecteds) {
                            if (ibas.strings.isEmpty(selected.itemCode)) {
                                continue;
                            }
                            if (ibas.objects.isNull(caller)) {
                                caller = this.editData.salesCreditNoteItems.create();
                            }
                            ...
                            this.chooseSalesCreditNoteItemMaterial(caller, [condition]);
                            caller = null;
                        }
                        if (this.editData.salesCreditNoteItems.length > count) {
                            this.view.showSalesCreditNoteItems(this.editData.salesCreditNoteItems.filterDeleted());
                        }
                    }
                });
