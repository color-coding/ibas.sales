...
namespace sales {
    export namespace app {
        ...
        export class SalesOrderListApp extends ibas.BOListApplication<ISalesOrderListView, bo.SalesOrder> {
            /** 应用标识 */
            static APPLICATION_ID: string = "6465a768-8834-487b-b3bb-f2c5affd4e0e";
            /** 应用名称 */
            static APPLICATION_NAME: string = "sales_app_salesorder_list";
            /** 业务对象编码 */
            ...
            private changeDocumentStatus(status: ibas.emDocumentStatus, datas: bo.SalesOrder[]): void {
                if (!(datas?.length > 0)) {
                    this.messages(ibas.emMessageType.WARNING, ibas.i18n.prop("shell_please_chooose_data",
                                                ...
                                                boRepository.saveSalesOrder({
                                                    ...
                                                    onCompleted: (opRslt) => {
                                                        if (opRslt.resultCode !== 0) {
                                                            ...
                                                            let oData: bo.SalesOrder = datas.find(c => !ibas.objects.isNull(c) && c.docEntry === data.docEntry);
