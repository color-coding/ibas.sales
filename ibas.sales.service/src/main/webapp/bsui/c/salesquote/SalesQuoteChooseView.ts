...
namespace sales {
    export namespace ui {
        export namespace c {
             ...
             * 选择视图-销售订单
             */
            export class SalesQuoteChooseView extends ibas.BOQueryDialogViewWithPanel implements app.ISalesQuoteChooseView {
                /** 返回查询的对象 */
                get queryTarget(): any {
                ...
                draw(): any {
                    ...
                    this.table = new sap.extension.table.DataTable("", {
                        ...
                        columns: [
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesquote_docentry"),
                            ...
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesquote_approvalstatus"),
                                template: new sap.extension.m.Text("", {
                                }).bindProperty("bindingValue", {
                                    path: "approvalStatus",
                            ...
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesquote_canceled"),
                                template: new sap.extension.m.Text("", {
                                }).bindProperty("bindingValue", {
                                    path: "canceled",
                            ...
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesquote_documentstatus"),
                                template: new sap.extension.m.Text("", {
                                }).bindProperty("bindingValue", {
                                    path: "documentStatus",
                            ...
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesquote_customercode"),
                                template: new sap.extension.m.Text("", {
                                }).bindProperty("bindingValue", {
                                    path: "customerCode",
                            ...
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesquote_customername"),
                                template: new sap.extension.m.Text("", {
                                }).bindProperty("bindingValue", {
                                    path: "customerName",
                                    type: new sap.extension.data.Alphanumeric()
                            ...
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesquote_documentdate"),
                                template: new sap.extension.m.Text("", {
                                }).bindProperty("bindingValue", {
                                    path: "documentDate",
                            ...
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesquote_deliverydate"),
                                template: new sap.extension.m.Text("", {
                                }).bindProperty("bindingValue", {
                                    path: "deliveryDate",
                                    type: new sap.extension.data.Date()
                            ...
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesquote_ordertype"),
                            ...
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesquote_reference2"),
                                template: new sap.extension.m.Text("", {
                                }).bindProperty("bindingValue", {
                                    path: "reference2",
                                    type: new sap.extension.data.Alphanumeric()
                    ...
                    this.itemTable = new sap.extension.table.DataTable("", {
                        ...
                        nextDataSet(event: sap.ui.base.Event): void {
                            // 查询下一个数据集
                            let data: bo.SalesQuoteItem = event.getParameter("data");
                            if (ibas.objects.isNull(data)) {
                                return;
                            }
                            if (ibas.objects.isNull(that.lastCriteria)) {
                            ...
                            let docData: bo.SalesQuote = new bo.SalesQuote();
                        ...
                        rowDoubleClick(event: sap.ui.base.Event): void {
                            let selectItems: bo.SalesQuoteItem[] = ibas.arrays.create(event.getParameter("row")?.getBindingContext()?.getObject());
                            if (selectItems.length === 0) {
                                selectItems = that.itemTable.getSelecteds();
                            }
                            let selects: bo.SalesQuote[] = that.table.getModel().getData("rows");
                            for (let select of selects) {
                                let has: boolean = false;
                                for (let i: number = select.salesQuoteItems.length - 1; i >= 0; i--) {
                                    let item: any = select.salesQuoteItems[i];
                                    if (selectItems.find(c => c === item)) {
                                        has = true;
                                        continue;
                                    }
                                    select.salesQuoteItems.removeAt(i);
                                }
                                if (has === false) {
                                    select.markDeleted();
                                }
                                                    ...
                                                    menu: new sap.m.Menu("", {
                                                        items: [
                                                            this.customMenuItem = new sap.m.MenuItem("", {
                                                                ...
                                                                press: function (): void {
                                                                    let selecteds: ibas.IList<bo.SalesQuote> = that.table.getSelecteds();
                                                    ...
                                                    search(event: sap.ui.base.Event): void {
                                                        ...
                                                        if (source instanceof sap.m.SearchField) {
                                                            ...
                                                            let cCriteria: ibas.IChildCriteria = criteria.childCriterias.firstOrDefault(
                                                                c => c.propertyPath === bo.SalesQuote.PROPERTY_SALESQUOTEITEMS_NAME
                                                            );
                ...
                showDataItems(datas: bo.SalesQuote[]): void {
                    this.container.to(this.itemTable.getParent().getId());
                    let dataItems: ibas.IList<bo.SalesQuoteItem> = new ibas.ArrayList<bo.SalesQuoteItem>();
                    for (let item of datas) {
                        dataItems.add(item.salesQuoteItems);
                    }
