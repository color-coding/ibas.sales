...
namespace sales {
    export namespace ui {
        export namespace c {
             ...
             * 选择视图-销售订单
             */
            export class SalesOrderChooseView extends ibas.BOQueryDialogViewWithPanel implements app.ISalesOrderChooseView {
                /** 返回查询的对象 */
                get queryTarget(): any {
                ...
                draw(): any {
                    ...
                    this.table = new sap.extension.table.DataTable("", {
                        ...
                        columns: [
                            ...
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesorder_ordertype"),
                            ...
                            new sap.extension.table.DataColumn("", {
                                label: ibas.i18n.prop("bo_salesorder_dataowner"),
                    ...
                    this.itemTable = new sap.extension.table.DataTable("", {
                        ...
                        nextDataSet(event: sap.ui.base.Event): void {
                            // 查询下一个数据集
                            let data: bo.SalesOrderItem = event.getParameter("data");
                            if (ibas.objects.isNull(data)) {
                                return;
                            }
                            if (ibas.objects.isNull(that.lastCriteria)) {
                            ...
                            let docData: bo.SalesOrder = new bo.SalesOrder();
                        ...
                        rowDoubleClick(event: sap.ui.base.Event): void {
                            let selectItems: bo.SalesOrderItem[] = ibas.arrays.create(event.getParameter("row")?.getBindingContext()?.getObject());
                            if (selectItems.length === 0) {
                                selectItems = that.itemTable.getSelecteds();
                            }
                            let selects: bo.SalesOrder[] = that.table.getModel().getData("rows");
                            for (let select of selects) {
                                let has: boolean = false;
                                for (let i: number = select.salesOrderItems.length - 1; i >= 0; i--) {
                                    let item: any = select.salesOrderItems[i];
                                    if (selectItems.find(c => c === item)) {
                                        has = true;
                                        continue;
                                    }
                                    select.salesOrderItems.removeAt(i);
                                }
                                if (has === false) {
                                    select.markDeleted();
                                }
                                                    ...
                                                    menu: new sap.m.Menu("", {
                                                        items: [
                                                            this.customMenuItem = new sap.m.MenuItem("", {
                                                                ...
                                                                press: function (): void {
                                                                    let selecteds: ibas.IList<bo.SalesOrder> = that.table.getSelecteds();
                                                    ...
                                                    search(event: sap.ui.base.Event): void {
                                                        ...
                                                        if (source instanceof sap.m.SearchField) {
                                                            ...
                                                            let cCriteria: ibas.IChildCriteria = criteria.childCriterias.firstOrDefault(
                                                                c => c.propertyPath === bo.SalesOrder.PROPERTY_SALESORDERITEMS_NAME
                                                            );
                ...
                showDataItems(datas: bo.SalesOrder[]): void {
                    this.container.to(this.itemTable.getParent().getId());
                    let dataItems: ibas.IList<bo.SalesOrderItem> = new ibas.ArrayList<bo.SalesOrderItem>();
                    for (let item of datas) {
                        dataItems.add(item.salesOrderItems);
                    }
