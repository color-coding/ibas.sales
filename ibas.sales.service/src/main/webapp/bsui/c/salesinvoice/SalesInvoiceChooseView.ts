            ...
            export class SalesInvoiceChooseView extends ibas.BOQueryDialogViewWithPanel implements app.ISalesInvoiceChooseView {
                ...
                draw(): any {
                    ...
                    this.itemTable = new sap.extension.table.DataTable("", {
                        ...
                        nextDataSet(event: sap.ui.base.Event): void {
                            // 查询下一个数据集
                            let data: bo.SalesInvoiceItem = event.getParameter("data");
                            if (ibas.objects.isNull(data)) {
                                return;
                            }
                            if (ibas.objects.isNull(that.lastCriteria)) {
                            ...
                            let docData: bo.SalesInvoice = new bo.SalesInvoice();
                        ...
                        rowDoubleClick(event: sap.ui.base.Event): void {
                            let selectItems: bo.SalesInvoiceItem[] = ibas.arrays.create(event.getParameter("row")?.getBindingContext()?.getObject());
                            if (selectItems.length === 0) {
                                selectItems = that.itemTable.getSelecteds();
                            }
                            let selects: bo.SalesInvoice[] = that.table.getModel().getData("rows");
                                                    ...
                                                    search(event: sap.ui.base.Event): void {
                                                        ...
                                                        if (source instanceof sap.m.SearchField) {
                                                            ...
                                                            let cCriteria: ibas.IChildCriteria = criteria.childCriterias.firstOrDefault(
                                                                c => c.propertyPath === bo.SalesInvoice.PROPERTY_SALESINVOICEITEMS_NAME
                                                            );
