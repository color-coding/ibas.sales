 ...
 * 视图-SalesDelivery
 */
export class SalesDeliveryListView extends ibas.BOListView implements ISalesDeliveryListView {
    /** 返回查询的对象 */
    get queryTarget(): any {
                ...
                content: [
                    new sap.m.Button("", {
                        ...
                        press(oEvent: any): void {
                            ...
                            var editBo: bo.SalesDelivery = parentControl.getSwipedItem().getBindingContext().getObject();
    ...
    showData(datas: bo.SalesDelivery[]): void {
        ...
        if (!ibas.objects.isNull(model)) {
            // 已存在绑定数据，添加新的
            let hDatas: bo.SalesDelivery[] = (<any>model).getData();
