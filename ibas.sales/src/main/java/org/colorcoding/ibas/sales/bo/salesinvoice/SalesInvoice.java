...
public class SalesInvoice extends BusinessObject<SalesInvoice> implements ISalesInvoice, IDataOwnership, IApprovalData,
	 ...
	 * 序列化版本标记
	 */
	private static final long serialVersionUID = 6128177886047413365L;

	/**
	 * 当前类型
	 */
	private static final Class<?> MY_CLASS = SalesInvoice.class;

	/**
	 * 数据库表
	 */
	public static final String DB_TABLE_NAME = "${Company}_SL_OINV";
	 ...
	 * 业务对象名称
	 */
	public static final String BUSINESS_OBJECT_NAME = "SalesInvoice";
	 ...
	 * 毛利价格清单 属性
	 */
	@DbField(name = "GrossBase", type = DataType.NUMERIC, table = DB_TABLE_NAME)
	 ...
	 * 毛利 属性
	 */
	@DbField(name = "GrossProfit", type = DataType.DECIMAL, editType = EditType.SUM, table = DB_TABLE_NAME)
	 ...
	 * 属性名称-销售发票-行
	 */
	private static final String PROPERTY_SALESINVOICEITEMS_NAME = "SalesInvoiceItems";

	/**
	 * 销售发票-行的集合属性
	 * 
	 */
	public static final IPropertyInfo<ISalesInvoiceItems> PROPERTY_SALESINVOICEITEMS = registerProperty(
			PROPERTY_SALESINVOICEITEMS_NAME, ISalesInvoiceItems.class, MY_CLASS);
	 ...
	 * @return 值
	 */
	@XmlElementWrapper(name = PROPERTY_SALESINVOICEITEMS_NAME)
	@XmlElement(name = SalesInvoiceItem.BUSINESS_OBJECT_NAME, type = SalesInvoiceItem.class)
	public final ISalesInvoiceItems getSalesInvoiceItems() {
		return this.getProperty(PROPERTY_SALESINVOICEITEMS);
	}
	 ...
	 * @param value 值
	 */
	public final void setSalesInvoiceItems(ISalesInvoiceItems value) {
		this.setProperty(PROPERTY_SALESINVOICEITEMS, value);
	}
	 ...
	 * 属性名称-销售发票-预收款
	 */
	private static final String PROPERTY_SALESINVOICEDOWNPAYMENTS_NAME = "SalesInvoiceDownPayments";

	/**
	 * 销售发票-预收款的集合属性
	 * 
	 */
	public static final IPropertyInfo<ISalesInvoiceDownPayments> PROPERTY_SALESINVOICEDOWNPAYMENTS = registerProperty(
			PROPERTY_SALESINVOICEDOWNPAYMENTS_NAME, ISalesInvoiceDownPayments.class, MY_CLASS);
	 ...
	 * @return 值
	 */
	@XmlElementWrapper(name = PROPERTY_SALESINVOICEDOWNPAYMENTS_NAME)
	@XmlElement(name = SalesInvoiceDownPayment.BUSINESS_OBJECT_NAME, type = SalesInvoiceDownPayment.class)
	public final ISalesInvoiceDownPayments getSalesInvoiceDownPayments() {
		return this.getProperty(PROPERTY_SALESINVOICEDOWNPAYMENTS);
	}
	 ...
	 * @param value 值
	 */
	public final void setSalesInvoiceDownPayments(ISalesInvoiceDownPayments value) {
		this.setProperty(PROPERTY_SALESINVOICEDOWNPAYMENTS, value);
	}
	...
	protected void initialize() {
		...
		this.setSalesInvoiceItems(new SalesInvoiceItems(this));
		this.setShippingAddresss(new ShippingAddresss(this));
		this.setSalesInvoiceDownPayments(new SalesInvoiceDownPayments(this));
	...
	protected IBusinessRule[] registerRules() {
		return new IBusinessRule[] {
				...
				new BusinessRuleRequiredElements(PROPERTY_SALESINVOICEITEMS), // 要求有元素
				new BusinessRuleDocumentStatus(PROPERTY_DOCUMENTSTATUS, PROPERTY_SALESINVOICEITEMS,
						SalesInvoiceItem.PROPERTY_LINESTATUS), // 使用集合元素状态
				// 计算行-总计（含税）
				new BusinessRuleSumElements(PROPERTY_ITEMSLINETOTAL, PROPERTY_SALESINVOICEITEMS,
						SalesInvoiceItem.PROPERTY_LINETOTAL, new Predicate<SalesInvoiceItem>() {
							@Override
	...
	protected void reset() {
		...
		this.setDocumentStatus(emDocumentStatus.RELEASED);
		this.getSalesInvoiceItems().forEach(c -> c.setLineStatus(emDocumentStatus.RELEASED));
		this.getSalesInvoiceDownPayments().forEach(c -> c.setLineStatus(emDocumentStatus.RELEASED));
	}
	...
	public IBusinessLogicContract[] getContracts() {
		...
		contracts.add(new ICustomerCheckContract() {
			...
			public String getIdentifiers() {
				return SalesInvoice.this.getIdentifiers();
			}
			...
			public String getCustomerName() {
				return SalesInvoice.this.getCustomerName();
			}

			@Override
			public void setCustomerName(String value) {
				SalesInvoice.this.setCustomerName(value);
			}
		});
		// 分支检查
		...
		contracts.add(new IJournalEntryCreationContract() {
			...
			public boolean isOffsetting() {
				...
				if (SalesInvoice.this instanceof IBOTagDeleted) {
					IBOTagDeleted boTag = (IBOTagDeleted) SalesInvoice.this;
			...
			public DateTime getDocumentDate() {
				if (SalesInvoice.this.getCanceled() == emYesNo.YES) {
					return SalesInvoice.this.getCancellationDate();
				}
			...
			public JournalEntryContent[] getContents() {
				...
				if (!Decimals.VALUE_ONE.equals(SalesInvoice.this.getDiscount())) {
					for (JournalEntryContent item : jeContents) {
						...
						if (Ledgers.LEDGER_SALES_REVENUE_ACCOUNT.equals(item.getLedger())
							...
							item.setAmount(Decimals.multiply(item.getAmount(), SalesInvoice.this.getDiscount()));
						}
					}
				}
				// 送货地址-运费
				...
				for (ISalesInvoiceDownPayment item : SalesInvoice.this.getSalesInvoiceDownPayments()) {
					...
					jeContents.add(jeContent);
					// 预付款（取分录）
					jeContent = new SalesInvoiceDownPaymentAmount(item);
					jeContent.setCategory(Category.Debit);
					jeContent.setShortName(SalesInvoice.this.getCustomerCode());
					jeContent.setAmount(item.getDrawnTotal());
					jeContent.setCurrency(item.getPaymentCurrency());
