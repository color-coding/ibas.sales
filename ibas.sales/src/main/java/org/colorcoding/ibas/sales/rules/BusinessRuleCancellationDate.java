...
public class BusinessRuleCancellationDate extends BusinessRuleCommon {
	...
	protected void execute(BusinessRuleContext context) throws Exception {
		...
		if (cancellationDate == null) {
			cancellationDate = DateTime.MIN_VALUE;
		}
		if (canceled == emYesNo.YES) {
			if (DateTime.MIN_VALUE.equals(cancellationDate)) {
				context.getOutputValues().put(this.getCancellationDate(), DateTime.getToday());
			}
		} else {
			if (!DateTime.MIN_VALUE.equals(cancellationDate)) {
				context.getOutputValues().put(this.getCancellationDate(), DateTime.MIN_VALUE);
			}
		}
