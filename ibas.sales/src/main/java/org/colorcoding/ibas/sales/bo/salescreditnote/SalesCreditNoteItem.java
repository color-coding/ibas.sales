...
@XmlType(name = SalesCreditNoteItem.BUSINESS_OBJECT_NAME, namespace = MyConfiguration.NAMESPACE_BO)
public class SalesCreditNoteItem extends BusinessObject<SalesCreditNoteItem> implements ISalesCreditNoteItem,
		IBusinessLogicsHost, IBOTagDeleted, IBOTagCanceled, IBOUserFields, IJECPropertyValueGetter {

	/**
	 * 序列化版本标记
	 */
	private static final long serialVersionUID = 485294099552251261L;

	/**
	 * 当前类型
	 */
	private static final Class<?> MY_CLASS = SalesCreditNoteItem.class;

	/**
	 * 数据库表
	 */
	public static final String DB_TABLE_NAME = "${Company}_SL_RIN1";
	 ...
	 * 业务对象名称
	 */
	public static final String BUSINESS_OBJECT_NAME = "SalesCreditNoteItem";
	 ...
	 * 基础数量 属性
	 */
	@DbField(name = "BasisQty", type = DataType.DECIMAL, editType = EditType.QUANTITY, table = DB_TABLE_NAME)
	 ...
	 * 行标志号 属性
	 */
	@DbField(name = "LineSign", type = DataType.ALPHANUMERIC, table = DB_TABLE_NAME)
	 ...
	 * 父项行标志号 属性
	 */
	@DbField(name = "ParentSign", type = DataType.ALPHANUMERIC, table = DB_TABLE_NAME)
	 ...
	 * 退货成本（本币） 属性
	 */
	@DbField(name = "RetCost", type = DataType.DECIMAL, editType = EditType.PRICE, table = DB_TABLE_NAME)
	 ...
	 * 毛利价格 属性
	 */
	@DbField(name = "GrossPrice", type = DataType.DECIMAL, editType = EditType.PRICE, table = DB_TABLE_NAME)
	...
	public IBusinessLogicContract[] getContracts() {
		...
		contracts.add(new ITaxGroupCheckContract() {
			...
			public String getTax() {
				return SalesCreditNoteItem.this.getTax();
			}

			@Override
			public BigDecimal getTaxRate() {
				return SalesCreditNoteItem.this.getTaxRate();
			}
		});
		// 物料及仓库检查
		...
		contracts.add(new IMaterialCatalogCheckContract() {
			...
			public void setCatalogCode(String value) {
				SalesCreditNoteItem.this.setCatalogCode(value);
			}
			...
			public String getCatalogCode() {
				return SalesCreditNoteItem.this.getCatalogCode();
			}
			...
			public String getBusinessPartnerCode() {
				return SalesCreditNoteItem.this.parent.getCustomerCode();
			}
		});
		// 基于单据完成数量
		if (!MyConfiguration.applyVariables(SalesOrder.BUSINESS_OBJECT_CODE)
			...
			contracts.add(new IDocumentQuantityClosingContract() {
				...
				public BigDecimal getQuantity() {
					return SalesCreditNoteItem.this.getQuantity();
				}
		...
		if (!MyConfiguration.applyVariables(SalesReturn.BUSINESS_OBJECT_CODE).equals(this.getBaseDocumentType())
			...
			contracts.add(new IMaterialReceiptContract() {
				...
				public boolean isOffsetting() {
					...
					if (SalesCreditNoteItem.this.parent instanceof IApprovalData) {
						IApprovalData apData = (IApprovalData) SalesCreditNoteItem.this.parent;
				...
				public String getItemName() {
					return SalesCreditNoteItem.this.getItemDescription();
				}
				...
				public String getDocumentType() {
					return SalesCreditNoteItem.this.getObjectCode();
				}

				@Override
				public Integer getDocumentEntry() {
					return SalesCreditNoteItem.this.getDocEntry();
				}

				@Override
				public Integer getDocumentLineId() {
					return SalesCreditNoteItem.this.getLineId();
				}

				@Override
				public BigDecimal getQuantity() {
					return SalesCreditNoteItem.this.getInventoryQuantity();
				}

				@Override
				public String getUOM() {
					return SalesCreditNoteItem.this.getInventoryUOM();
				}

				@Override
				public DateTime getPostingDate() {
					return SalesCreditNoteItem.this.parent.getPostingDate();
				}

				@Override
				public DateTime getDeliveryDate() {
					return SalesCreditNoteItem.this.parent.getDeliveryDate();
				}

				@Override
				public DateTime getDocumentDate() {
					return SalesCreditNoteItem.this.parent.getDocumentDate();
				}
