...
public class SalesReturn extends BusinessObject<SalesReturn> implements ISalesReturn, IDataOwnership, IApprovalData,
		...
		IDocumentPaidTotalOperator, IDocumentCloseQuantityOperator, IDocumentPrintedOperator {
	/**
	 * 序列化版本标记
	 */
	...
	protected IBusinessRule[] registerRules() {
		return new IBusinessRule[] {
				...
				new BusinessRuleDocumentStatus(PROPERTY_DOCUMENTSTATUS, PROPERTY_SALESRETURNITEMS,
				...
				new BusinessRuleSumElements(PROPERTY_ITEMSLINETOTAL, PROPERTY_SALESRETURNITEMS,
						SalesReturnItem.PROPERTY_LINETOTAL, new Predicate<SalesReturnItem>() {
							@Override
		...
		return new IBusinessLogicContract[] {
				...
				new ICustomerCheckContract() {
					...
					public String getIdentifiers() {
						return SalesReturn.this.getIdentifiers();
					}
					...
					public String getCustomerName() {
						return SalesReturn.this.getCustomerName();
					}

					@Override
					public void setCustomerName(String value) {
						SalesReturn.this.setCustomerName(value);
					}
				},
				// 分支检查
				...
				new IJournalEntryCreationContract() {
					...
					public boolean isOffsetting() {
						...
						if (SalesReturn.this instanceof IBOTagDeleted) {
							IBOTagDeleted boTag = (IBOTagDeleted) SalesReturn.this;
					...
					public JournalEntryContent[] getContents() {
						...
						for (ISalesReturnItem line : SalesReturn.this.getSalesReturnItems()) {
							...
							if (SalesDeliveryCode.equals(line.getBaseDocumentType())
								...
								/** 不基于单据 **/
								if (line.getReturnCost().compareTo(Decimals.VALUE_ZERO) > 0) {
									/** 指定退货成本 **/
									// 销售成本科目
									jeContent = new JournalEntrySmartContent(line);
									jeContent.setCategory(Category.Debit);
									jeContent.setLedger(Ledgers.LEDGER_SALES_SHIPPED_GOODS_ACCOUNT);
									jeContent.setAmount(Decimals
											.multiply(line.getReturnCost(), line.getInventoryQuantity()).negate());// 成本总计
									jeContent
											.setCurrency(org.colorcoding.ibas.accounting.MyConfiguration.getConfigValue(
	...
	public Iterator<IDocumentClosingQuantityItem> getQuantityItems() {
		return new Iterator<IDocumentClosingQuantityItem>() {
			...
			public IDocumentClosingQuantityItem next() {
				this.index += 1;
				return SalesReturn.this.getSalesReturnItems().get(this.index);
			}

			@Override
			public boolean hasNext() {
				if (SalesReturn.this.getSalesReturnItems().isEmpty()) {
					return false;
				}
				int nIndex = this.index + 1;
				if (nIndex >= SalesReturn.this.getSalesReturnItems().size()) {
					return false;
				}
				return true;
			}
