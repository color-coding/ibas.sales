 ...
 *
 */
@LogicContract(ISalesDeliveryReturnContract.class)
public class SalesDeliveryReturnService extends SalesDeliveryService<ISalesDeliveryReturnContract> {

	@Override
	protected ISalesDelivery fetchBeAffected(ISalesDeliveryReturnContract contract) {
		return this.fetchBeAffected(contract.getBaseDocumentType(), contract.getBaseDocumentEntry());
	}

	@Override
	protected void impact(ISalesDeliveryReturnContract contract) {
	...
	@Override
	protected void revoke(ISalesDeliveryReturnContract contract) {
		if (this.getBeAffected() == null) {
			return;
		}
		ISalesDeliveryItem orderItem = this.getBeAffected().getSalesDeliveryItems()
