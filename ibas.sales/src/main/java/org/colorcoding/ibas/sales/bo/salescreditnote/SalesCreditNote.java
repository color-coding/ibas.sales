...
public class SalesCreditNote extends BusinessObject<SalesCreditNote> implements ISalesCreditNote, IDataOwnership,
		...
		return new IBusinessLogicContract[] {
				...
				new IJournalEntryCreationContract() {
					...
					public JournalEntryContent[] getContents() {
						...
						// 单据折扣不是1
						if (!Decimals.VALUE_ONE.equals(SalesCreditNote.this.getDiscount())) {
							for (JournalEntryContent item : jeContents) {
								// 行税前总计和行税 × 折扣
								// 注：贷项以"销售贷方"冲抵收入，与销项税同步打折；
								...
								if (Ledgers.LEDGER_SALES_SALES_CREDIT_ACCOUNT.equals(item.getLedger())
									...
									item.setAmount(
											Decimals.multiply(item.getAmount(), SalesCreditNote.this.getDiscount()));
								}
							}
						}
						// 送货地址-运费
						...
						jeContent.setAmount(SalesCreditNote.this.getDiffAmount().negate());
	...
	public Object getValue(String property) {
		...
		case Ledgers.CONDITION_PROPERTY_MATERIAL:
			String[] items = new String[this.getSalesCreditNoteItems().size()];
			for (int i = 0; i < items.length; i++) {
				items[i] = this.getSalesCreditNoteItems().get(i).getItemCode();
			}
