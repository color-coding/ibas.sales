...
@XmlType(name = SalesReserveInvoice.BUSINESS_OBJECT_NAME, namespace = MyConfiguration.NAMESPACE_BO)
@XmlRootElement(name = SalesReserveInvoice.BUSINESS_OBJECT_NAME, namespace = MyConfiguration.NAMESPACE_BO)
@BusinessObjectUnit(code = SalesReserveInvoice.BUSINESS_OBJECT_CODE)
public class SalesReserveInvoice extends BusinessObject<SalesReserveInvoice>
		implements ISalesReserveInvoice, IDataOwnership, IApprovalData, IPeriodData, IProjectData, IBOTagDeleted,
	 ...
	 * 当前类型
	 */
	private static final Class<?> MY_CLASS = SalesReserveInvoice.class;

	/**
	 * 数据库表
	 */
	public static final String DB_TABLE_NAME = "${Company}_SL_ORIV";

	/**
	 * 业务对象编码
	 */
	public static final String BUSINESS_OBJECT_CODE = "${Company}_SL_SALESRSVINVOICE";

	/**
	 * 业务对象名称
	 */
	public static final String BUSINESS_OBJECT_NAME = "SalesReserveInvoice";
	 ...
	 * 
	 */
	public static final IPropertyInfo<ISalesReserveInvoiceItems> PROPERTY_SALESRESERVEINVOICEITEMS = registerProperty(
			PROPERTY_SALESRESERVEINVOICEITEMS_NAME, ISalesReserveInvoiceItems.class, MY_CLASS);
	 ...
	 * @return 值
	 */
	@XmlElementWrapper(name = PROPERTY_SALESRESERVEINVOICEITEMS_NAME)
	@XmlElement(name = SalesReserveInvoiceItem.BUSINESS_OBJECT_NAME, type = SalesReserveInvoiceItem.class)
	public final ISalesReserveInvoiceItems getSalesReserveInvoiceItems() {
		return this.getProperty(PROPERTY_SALESRESERVEINVOICEITEMS);
	}
	 ...
	 * @param value 值
	 */
	public final void setSalesReserveInvoiceItems(ISalesReserveInvoiceItems value) {
		this.setProperty(PROPERTY_SALESRESERVEINVOICEITEMS, value);
	}
	...
	protected void initialize() {
		...
		this.setSalesReserveInvoiceItems(new SalesReserveInvoiceItems(this));
	...
	protected IBusinessRule[] registerRules() {
		return new IBusinessRule[] {
				...
				new BusinessRuleRequiredElements(PROPERTY_SALESRESERVEINVOICEITEMS), // 要求有元素
				new BusinessRuleDocumentStatus(PROPERTY_DOCUMENTSTATUS, PROPERTY_SALESRESERVEINVOICEITEMS,
						SalesReserveInvoiceItem.PROPERTY_LINESTATUS), // 使用集合元素状态
				// 计算行-总计（含税）
	...
	protected void reset() {
		...
		this.setDocumentStatus(emDocumentStatus.RELEASED);
		this.getSalesReserveInvoiceItems().forEach(c -> c.setLineStatus(emDocumentStatus.RELEASED));
	}
		...
		return new IBusinessLogicContract[] {
				...
				new ICustomerCheckContract() {
					...
					public String getIdentifiers() {
						return SalesReserveInvoice.this.getIdentifiers();
					}

					@Override
					public String getCustomerCode() {
						return SalesReserveInvoice.this.getCustomerCode();
					}

					@Override
					public String getCustomerName() {
						return SalesReserveInvoice.this.getCustomerName();
					}

					@Override
					public void setCustomerName(String value) {
						SalesReserveInvoice.this.setCustomerName(value);
					}
				},
				// 分支检查
				new IBranchCheckContract() {
					...
					public String getBranch() {
						return SalesReserveInvoice.this.getBranch();
					}
				},
				// 创建分录
				new IJournalEntryCreationContract() {
					...
					public String getBaseDocumentType() {
						return SalesReserveInvoice.this.getObjectCode();
					}

					@Override
					public Integer getBaseDocumentEntry() {
						return SalesReserveInvoice.this.getDocEntry();
					}
					...
					public String getReference1() {
						return SalesReserveInvoice.this.getReference1();
					}

					@Override
					public String getReference2() {
						return SalesReserveInvoice.this.getReference2();
					}
					...
					public JournalEntryContent[] getContents() {
						...
						// 单据折扣不是1
						if (!Decimals.VALUE_ONE.equals(SalesReserveInvoice.this.getDiscount())) {
							for (JournalEntryContent item : jeContents) {
								...
								if (Ledgers.LEDGER_SALES_REVENUE_ACCOUNT.equals(item.getLedger())
									...
									item.setAmount(Decimals.multiply(item.getAmount(),
											SalesReserveInvoice.this.getDiscount()));
								}
							}
						}
						// 送货地址-运费（运费不折扣）
						...
						jeContent.setAmount(SalesReserveInvoice.this.getDiffAmount());
	...
	public boolean isSmartDocumentStatus() {
		// 付款后的状态改变，单行均完成时
		for (ISalesReserveInvoiceItem item : this.getSalesReserveInvoiceItems()) {
	...
	public Iterator<IDocumentClosingAmountItem> getAmountItems() {
		return new Iterator<IDocumentClosingAmountItem>() {
			...
			public IDocumentClosingAmountItem next() {
				this.index += 1;
				return SalesReserveInvoice.this.getSalesReserveInvoiceItems().get(this.index);
			}

			@Override
			public boolean hasNext() {
				if (SalesReserveInvoice.this.getSalesReserveInvoiceItems().isEmpty()) {
					return false;
				}
				int nIndex = this.index + 1;
				if (nIndex >= SalesReserveInvoice.this.getSalesReserveInvoiceItems().size()) {
					return false;
				}
				return true;
			}
