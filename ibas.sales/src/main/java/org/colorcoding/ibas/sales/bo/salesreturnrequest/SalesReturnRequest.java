...
@XmlType(name = SalesReturnRequest.BUSINESS_OBJECT_NAME, namespace = MyConfiguration.NAMESPACE_BO)
@XmlRootElement(name = SalesReturnRequest.BUSINESS_OBJECT_NAME, namespace = MyConfiguration.NAMESPACE_BO)
@BusinessObjectUnit(code = SalesReturnRequest.BUSINESS_OBJECT_CODE)
public class SalesReturnRequest extends BusinessObject<SalesReturnRequest> implements ISalesReturnRequest,
	 ...
	 * 序列化版本标记
	 */
	private static final long serialVersionUID = 1156124875782694443L;

	/**
	 * 当前类型
	 */
	private static final Class<?> MY_CLASS = SalesReturnRequest.class;

	/**
	 * 数据库表
	 */
	public static final String DB_TABLE_NAME = "${Company}_SL_ORRR";
	 ...
	 * 业务对象名称
	 */
	public static final String BUSINESS_OBJECT_NAME = "SalesReturnRequest";
	 ...
	 * 属性名称-销售退货请求-行
	 */
	private static final String PROPERTY_SALESRETURNREQUESTITEMS_NAME = "SalesReturnRequestItems";

	/**
	 * 销售退货请求-行的集合属性
	 * 
	 */
	public static final IPropertyInfo<ISalesReturnRequestItems> PROPERTY_SALESRETURNREQUESTITEMS = registerProperty(
			PROPERTY_SALESRETURNREQUESTITEMS_NAME, ISalesReturnRequestItems.class, MY_CLASS);
	 ...
	 * @return 值
	 */
	@XmlElementWrapper(name = PROPERTY_SALESRETURNREQUESTITEMS_NAME)
	@XmlElement(name = SalesReturnRequestItem.BUSINESS_OBJECT_NAME, type = SalesReturnRequestItem.class)
	public final ISalesReturnRequestItems getSalesReturnRequestItems() {
		return this.getProperty(PROPERTY_SALESRETURNREQUESTITEMS);
	}
	 ...
	 * @param value 值
	 */
	public final void setSalesReturnRequestItems(ISalesReturnRequestItems value) {
		this.setProperty(PROPERTY_SALESRETURNREQUESTITEMS, value);
	}
	...
	protected IBusinessRule[] registerRules() {
		return new IBusinessRule[] {
				...
				new BusinessRuleDocumentStatus(PROPERTY_DOCUMENTSTATUS, PROPERTY_SALESRETURNREQUESTITEMS,
						SalesReturnRequestItem.PROPERTY_LINESTATUS), // 使用集合元素状态
				// 计算行-总计（含税）
		...
		return new IBusinessLogicContract[] {
				...
				new ICustomerCheckContract() {
					...
					public String getIdentifiers() {
						return SalesReturnRequest.this.getIdentifiers();
					}

					@Override
					public String getCustomerCode() {
						return SalesReturnRequest.this.getCustomerCode();
					}

					@Override
					public String getCustomerName() {
						return SalesReturnRequest.this.getCustomerName();
					}

					@Override
					public void setCustomerName(String value) {
						SalesReturnRequest.this.setCustomerName(value);
					}
				},
				// 分支检查
				new IBranchCheckContract() {
					...
					@Override
					public String getIdentifiers() {
						return SalesReturnRequest.this.toString();
					}

					@Override
					public String getBranch() {
						return SalesReturnRequest.this.getBranch();
					}
				}
	...
	public Iterator<IDocumentClosingQuantityItem> getQuantityItems() {
		return new Iterator<IDocumentClosingQuantityItem>() {
			...
			public IDocumentClosingQuantityItem next() {
				this.index += 1;
				return SalesReturnRequest.this.getSalesReturnRequestItems().get(this.index);
			}

			@Override
			public boolean hasNext() {
				if (SalesReturnRequest.this.getSalesReturnRequestItems().isEmpty()) {
					return false;
				}
				int nIndex = this.index + 1;
				if (nIndex >= SalesReturnRequest.this.getSalesReturnRequestItems().size()) {
					return false;
				}
				return true;
			}
